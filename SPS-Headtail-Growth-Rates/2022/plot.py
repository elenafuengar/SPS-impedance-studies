...
dfHT = pd.read_csv('qpvscan-q26-resonatorf26/qpvscan.csv', sep=' ').sort_values(by='QPV') 
qpvvals = dfHT['QPV']
grvals = dfHT['slope']*1e3
errors = dfHT['error']*1e3

ax.plot(qpvvals, grvals, 'o-', color=colors[3], mec=colors[3], ms=4, label='pyHT q26 + resonator')
ax.fill_between(qpvvals, grvals-errors, grvals+errors, color=colors[3], ec='k', alpha=0.3)
ax.set_xlabel('Chromaticity $Q\'_v$ value')
ax.set_ylabel('Growthrate [1e3/nturns]')
ax.invert_xaxis()

axx = ax.twiny()
axx.plot(-qpvvals*1.6, grvals, color=colors[2], ls='None')
axx.set_xlabel(r'Chromaticity frequency $f_{\xi}$ [GHz]')
axx.set_xlim(0, 2*1.6)

##add non-linear chroma errorbar 
dfHT = pd.read_csv('qpvscan-q26-qpp13-qppp1e6/qpvscan.csv', sep=' ').sort_values(by='QPV') 
grvals_qpp = dfHT['slope']*1e3

#add nominal q26 non linear
dfHT = pd.read_csv('qpvscan-nonlinearq26/qpvscan.csv', sep=' ').sort_values(by='QPV') 
qpvvals = dfHT['QPV']
grvals = dfHT['slope']*1e3
errors = np.ones_like(qpvvals) #dfHT['error']*1e3 

ax.plot(qpvvals, grvals, 'o-k', mec='k', label='pyHT q26 wake')
#ax.fill_between(qpvvals, grvals, grvals_qpp, color='k', alpha=0.3, label='$Q''_v$ uncertainty')
ax.fill_between(qpvvals, grvals-errors, grvals+errors, color='k', alpha=0.2, label='$Q''_v$ uncertainty')
