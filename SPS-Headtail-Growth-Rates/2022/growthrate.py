...
class GrowthRate():
    ...
    def __init__(self, file='bunchmonitor.h5', nperseg=65, noverlap=50, 
                 turn_min=None , turn_max=None, interactive=True, grstart=None,
                 machine='SPS', grmin=1e-5, **kwargs):
    ...
    def get_data(self):
        ...
        self.data = f['Bunch']
        self.vpos = np.array(f['Bunch']['mean_y'])
        self.turns = np.arange(len(self.vpos))
    ...
    def get_data_parquet(self):
        ...
        self.data = d
        try:
            self.vpos = d['SPS.BQ.KICKED/ContinuousAcquisition']['value']['rawDataV']
        except:
    ...
    def get_QP_knob(self, cycle_offset=4015, cycle_start=200):
    ...
    def get_grstart(self):
        ...
        ax.plot(self.turns, self.vpos, color='b')
        ...
        if self.turn_max is not None and self.turn_min is not None:
            ax.set_ylim(ymin=self.vpos[int(self.turn_min):int(self.turn_max)].min(), ymax=self.vpos[int(self.turn_min):int(self.turn_max)].max())
    ...
    def MWFFT_analysis(self):
        ...
        xend = np.argmax(self.vpos[xpos])+xstart+1
        self.beam_unstable = True
        ...
        if (xend-xstart) < self.nperseg:
            ...
            self.error = 0.
            self.beam_unstable = False
            print('Beam is not unstable, slope set to 0.0')
            return

        #perform MWFFT (scipy.signal)
        pad = 5
        vpos_pad = self.vpos[xstart-pad:xend+pad]
        MWFFT_all = scipy.signal.stft(self.vpos, fs=1.0, window='hann', nperseg=self.nperseg, noverlap=self.noverlap, nfft=None, detrend=False, return_onesided=True, boundary='zeros', padded=True, axis=- 1)
        MWFFT = scipy.signal.stft(vpos_pad, fs=1.0, window='hann', nperseg=self.nperseg, noverlap=self.noverlap, nfft=None, detrend=False, return_onesided=True, boundary='zeros', padded=True, axis=- 1)
        ...
        try:
            ...
            if self.slope < 0:
                print(f'gr.slope is negative due to a bad fit and will be set to 0.0')
                self.error = 0.
                self.slope = 0.
                self.beam_unstable = False
            elif self.slope < self.grmin:
                print(f'gr.slope is too small and will be set to 0.0')
                self.error = 0.
                self.slope = 0.
                self.beam_unstable = False
            else:
            ...
            print('Linear Fit could not be performed, slope set to 0.0')
            self.error = 0.
            self.slope = 0.
            self.beam_unstable = False
    ...
    def plot(self):
        ...
        ax1.plot(self.turns, self.vpos, color='b', alpha = 0.5)
        ax1.plot(self.turns[xstart:xend], self.vpos[xstart:xend], color='b') 
        ...
        ax1.set_ylim(-np.max(self.vpos[xstart:xend])*1.2, np.max(self.vpos[xstart:xend])*1.2)
